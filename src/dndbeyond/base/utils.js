...
function injectCustomRollButton() {
    ...
    // In Enounter Builder there is no menu button, so we can skip that particular check.
    // In Character Sheet we must wait for initialization of both buttons.
    if (!rollButton || !menuButton && !document.querySelector("div[name=encounter-builder]"))
        return;
